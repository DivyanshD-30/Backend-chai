...
const registerUser = asyncHandler( async (req,res) => {
    // get the user detail from the user
...
const getCurrentUserDetails = asyncHandler( async (req,res) => {
    return res.status(200).json(new ApiResponse(
        200,req.user,"Current user details fetched successfully"
    ));
});
...
const updateUserAvatar = asyncHandler( async (req,res) => {
    ...
    const avatar = await upploadOnCloudinary(avatarLocalPath,"Avatar");
...
const updateUserCoverImage = asyncHandler( async (req,res) => {
    ...
    const coverImage = await upploadOnCloudinary(coverImageLocalPath,"Cover Image");

    if(!coverImage.url){
        throw new ApiError(500,"Error in uploading cover image file");
    }
    const user = await User.findByIdAndUpdate(
        req.user?._id,
        {
